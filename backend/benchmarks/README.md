...
эталоны датасета неоднородны — 82 листа из 123 покрыты полностью, 14 менее чем на
20 %. Сводная медиана 19.9 % смешивает два знаменателя; по подмножеству с полным
эталоном измерено 22.4 % площади листа, полнота после заполнения 95.1 %.

Почему измеренных ячеек столько: земля составляет медианно 12.3 % точек облака,
то есть 0.20 ground-точки на м². На метровой сетке это одна точка на пять ячеек —
занятость ограничена самими данными, а не алгоритмом.
