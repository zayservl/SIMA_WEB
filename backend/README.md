...
│   └── conftest.py
├── relief_demo.ipynb            # Демонстрация сервиса рельефа на одном тайле
├── relief_benchmark.ipynb       # Скорость, точность и полнота на 123 листах Юильского
├── relief_s3_yuilskiy.ipynb     # Тот же конвейер на данных из бакета (Юильский)
├── relief_s3_yuzhno_zimniy.ipynb # То же на Южно-Зимнем (абсолютные Z, блоки 1×1 км)
├── forest_s3_yuilskiy.ipynb     # ЦМД и деревья на Юильском: сверка с эталоном СИМА 1.44
├── benchmarks/                  # Харнессы прогонов: relief_bench.py, s3_relief.py, результаты
...
|---|---|
| `relief_demo.ipynb` | полный конвейер на одном тайле: оценка материалов → ЦМР → ЦММ → сглаживание → производные → векторы, плюс сравнение методов классификации земли и силы сглаживания. Датасеты `demo` и `test` |
| `relief_benchmark.ipynb` | 123 листа Юильского с эталонной ЦМР: время листа, точность против эталона, полнота растра против покрытия ЛАС, карты каждого десятого листа |
| `relief_s3_yuilskiy.ipynb` | 100 листов прямо из бакета; абсолютные Z восстанавливаются из ТЛО по эталонной ЦМР (`RESTORE_Z`) |
| `relief_s3_yuzhno_zimniy.ipynb` | 30 блоков нарезки TerraScan с абсолютными Z и заданной вручную СК |
| `forest_s3_yuilskiy.ipynb` | ЦМД, вершины деревьев и кроны; сверка с выгрузкой СИМА 1.44 |

Тяжёлые прогоны выносятся из ноутбуков в харнессы `benchmarks/relief_bench.py` и
`benchmarks/s3_relief.py` — результаты пишутся в JSONL, ноутбук их отрисовывает.
Прежние версии ноутбуков — в `notebooks_v1/`.
...
| TIN | DXF | ✅ |
| Маска измеренных ячеек ЦМР | GeoTIFF (uint8) | по запросу |

Маска измеренных ячеек (`ReliefService(save_measured_mask=True)`, слой
`dtm_measured`) отмечает, какие значения ЦМР получены растеризацией точек, а какие
достроены заполнением пустот. После заполнения различить их по самому растру
невозможно, поэтому маска снимается сразу после растеризации. Нужна для оценки
точности и для отчёта заказчику о полноте модели.
...
- **Гидровыравнивание срабатывает не только на водоёмах** — `hydro_flatten=True` по умолчанию, и крупная пустота съёмки, прошедшая проверки `hydro.flatten_water_voids`, получает плоскую отметку вместо интерполяции. На тайле `P-42-041-239-g` это даёт 3.2 % ячеек с ошибкой более метра и поднимает RMSE по всем ячейкам с 0.100 до 0.379 м; на точность по измеренным ячейкам не влияет вовсе (0.065 м в обоих случаях), потому что затрагивает только достроенные. Параметр вынесен в контракт (`DerivativesParams.hydro_flatten`, `DsmParams.hydro_flatten`) — на участках без водоёмов его следует выключать.
- **Ground-точек меньше, чем ячеек растра при 1 м/пиксель.** В облаках Юильского земля составляет медианно 12.3 % точек (диапазон 5.4–40.6 %), что даёт 0.20 ground-точки на м². На метровой сетке это одна точка на пять ячеек: пуассоновская оценка занятости — 18 %, фактически измерено 22.4 % площади листа (по 82 листам с полным эталоном), остальное достраивается заполнением пустот до 95 %. Это свойство данных, а не дефект конвейера: при 2 м/пиксель занятость поднимается до ~55 %, при 3 м — до ~84 %. Разрешение ЦМР следует выбирать по плотности ground-точек, а не по разрешению АФС.
- **Чем гуще облако, тем меньше в нём земли.** По трети датасета с наибольшей плотностью (2.05–3.05 т/м²) доля ground-точек 9.4 %, по трети с наименьшей (0.23–1.65 т/м²) — 28.1 %. Плотное облако означает густой полог, сквозь который до земли доходит меньшая доля импульсов, поэтому измеренная часть ЦМР там *ниже*: 17.5 % против 38.9 %.
- **Эталонные ЦМР датасета неоднородны по полноте.** 82 листа из 123 покрыты полностью, 14 — менее чем на 20 % своей рамки. На разреженных листах `coverage_pct` считается от малого знаменателя и с остальными не сопоставим: измерено 36.8 % валидных ячеек эталона, но лишь 3.9 % площади листа. Сводные медианы по всему датасету смешивают два знаменателя — сравнивать нужно внутри подмножества с полным эталоном.
