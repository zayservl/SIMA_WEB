...
1. **Заполнение пустот — общий многопроходный механизм** (`sima_dem_core/raster/holes.py::fill_voids`), одинаковый для ЦМР (`ground.py`), сглаживания (`smooth.py`) и ЦММ (`dsm.py`). Заполняются внутренние дырки (окружённые валидными данными, `binary_fill_holes`) плюс пустоты не далее `edge_extrapolation_m` от данных. Маска пересчитывается после каждого прохода: часть пустот (характерно — водоёмы, сообщавшиеся с внешней пустотой протокой) замыкается в дыру только после заполнения соседних, и за один проход остаётся незаполненной. Наружный допуск считается от исходной маски и с проходами не нарастает. Валидные ячейки не изменяются ни при каком числе проходов — гарантируется финальной перезаписью `out[valid] = array[valid]`. Число проходов: `fill_passes` (по умолчанию 3; `1` — прежнее однопроходное поведение).
