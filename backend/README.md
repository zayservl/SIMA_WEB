...
проверяется и не попадает в `failed_tiles` — упавшим считается только тайл,
который не удалось открыть. Сравнения СК каталога АФС с каталогом ВЛС в
библиотеке тоже нет: `AfsReport.crs` и `VlsReport.crs` возвращаются рядом, а
решение о совместимости остаётся вызывающему коду (в прототипе это делает
`src/pages/Upload.tsx::pairStatus`).
...
### 3. Где АФС и ВЛС встречаются в одном расчёте
...
### 4. Высоты: ТЛО против абсолютных отметок
...
| Ожидание | Факт |
|---|---|
| Приведение ВЛС и АФС к общей СК | Нет. Ни репроекции, ни проверки совпадения |
