...
за границу съёмки) область **без единого возврата любого класса** площадью не
меньше `min_area_m2`. Внутри полосы сканирования такие области — практически
всегда вода: любая суша даёт возвраты хотя бы от растительности. Порог по
умолчанию — из стандарта.

Маску «нет возвратов» обязан передать вызывающий (`water`), и для ЦМР это
существенно: её валидная маска — редкая сетка ячеек с ground-точками (10–30 %
листа), поэтому пустотой формально оказывается почти весь лист. Без ограничения
по возвратам такая «лакуна» проходит отбор по площади и выравнивается в одну
плоскость — артефакт на несколько метров, а не водоём.
...
def flatten_water_voids(
    ...
    Args:
        ...
        water: ограничение на кандидатов — маска водоёмов (брейклайны, слой
            анализа воды) либо маска ячеек без единого возврата ВЛС. Без неё
            кандидатом считается любая пустота, что на ЦМР неверно: её валидная
            маска — редкая сетка ground-ячеек, и всё пространство между ними
            образует одну гигантскую «лакуну».
        min_area_m2: минимальная площадь водоёма; по умолчанию порог 3DEP.
            0 — не отбирать по площади (осмысленно с готовыми брейклайнами).
